...
% % Original Agility Robotics Scope
% if ~isempty(outputs)
%     log.outputs.t = outputs.data(:,end);
%     log.outputs.vectorNavOrientation = outputs.data(:,1:4);
%     log.outputs.vectorNavAngularVelocity = outputs.data(:,5:7);
%     log.outputs.motorPosition = outputs.data(:,8:17);
%     log.outputs.motorVelocity = outputs.data(:,18:27);
%     log.outputs.jointPosition = outputs.data(:,28:31);
%     log.outputs.jointVelocity = outputs.data(:,32:35);
% end
...
%% Parse EKF
if ~isempty(ekf)
    log.ekf.t = ekf.data(:,end);
    log.ekf.velocity_ekf = ekf.data(:,1:3);
    log.ekf.velocity_kinematic = ekf.data(:,4:6);
    log.ekf.mu = ekf.data(:,7:28);
    log.ekf.enabled = ekf.data(:,29);
end
